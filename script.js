document.addEventListener('DOMContentLoaded', function () {
    ...
    const updateNavbarAuth = () => {
        ...
        // Find the "Sign In" button list item
        ...
        if (currentUser && signInItem) {
            // Replace Sign In with User Menu
    ...
    if (resultsContainer) {
        ...
        if (summaryElement) {
            summaryElement.textContent = `Showing flights from ${fromParam} to ${toParam}` + (dateParam ? ` for ${dateParam}` : '');
        }

        // Fetch Data
        const flights = DataManager.searchFlights(fromParam, toParam, dateParam);
        ...
        if (flights.length === 0) {
            ...
            flights.forEach((flight, i) => {
                // Determine icons based on airline name (mock logic)
                let icon = 'bi-airplane-engines';
                if (flight.airline.includes('Sky')) icon = 'bi-cloud-haze2';
                if (flight.airline.includes('Global')) icon = 'bi-globe';
...
window.goToPayment = function (flightId, airline, duration, depTime, arrTime, price, from, to, date) {
    // Check if logged in
    const user = DataManager.getCurrentUser();
    if (!user) {
        // Store intended destination in sessionStorage? (skip for now to keep simple)
...
if (paymentForm) {
    ...
    paymentForm.addEventListener('submit', function (e) {
        ...
        setTimeout(() => {
            ...
            DataManager.createBooking(bookingData);
